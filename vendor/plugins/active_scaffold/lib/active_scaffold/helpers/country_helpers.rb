    ...
    module CountryHelpers
      ...
      def country_options_for_select(selected = nil, priority_countries = nil)
        ...
        if priority_countries
          ...
          disabled_field = content_tag(:option, '', :disabled => 'disabled', :selected => "#{selected.nil? ? 'selected' : ''}")
          country_options += "#{disabled_field}\n"
        end

        return country_options + options_for_select(COUNTRIES.collect {|country| [I18n.t("countries.#{country}", :default => country.to_s.titleize), I18n.t("countries.#{country}", :default => country.to_s.titleize)]}, selected)
      end
