  ...
  context "concerning the drafting service" do
    ...
    context "ready improvement proposal that went Tr unchanged" do
      ...
      should("send an approval email") do
        ...
        assert_equal [@statement_2.current_document_in_original_language.author.email], email.to
    ...
    context "approved improvement proposal that went Ta unchanged" do
      ...
      should("send passed email") do
        ...
        assert !ActionMailer::Base.deliveries.empty?
        assert_equal [@statement_1.current_document_in_original_language.author.email], email.to
    ...
    context " improvement proposal incorporation in proposal" do
      setup {
        ...
        old_doc = @statement_2.current_document_in_original_language

        @statement_2.current_document_in_original_language.update_attribute(:current, false)
        @statement_2.user_echos << UserEcho.new(:echo => @statement_2.find_or_create_echo,
                                                :user => users(:red),
                                                :supported => true)
        @statement_2.user_echos << UserEcho.new(:echo => @statement_2.find_or_create_echo,
        ...
        @statement_2.add_statement_document :original_language_id => @statement_2.original_language.id,
                                            :title => old_doc.title,
                                            :text => old_doc.text + @statement_1.current_document_in_original_language.text,
                                            :statement_id => @statement_2.statement_id,
                                            :language_id => old_doc.language.id,
                                            :current => true,
                                            :author_id => @statement_1.current_document_in_original_language.author.id,
