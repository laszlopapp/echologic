class EnumValue < ActiveRecord::Base
  ...
  belongs_to :enum_key
  validates_presence_of :enum_key_id, :value, :code
    ...
    def all_by_code
      ...
      begin
        @all_by_code = all.inject({}) { |memo, item| 
          memo[item.enum_key.code] = memo[item.enum_key.code].nil? ? Hash[item.code => item] : memo[item.enum_key.code].merge({item.code => item})
          memo}.freeze
