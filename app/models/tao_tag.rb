class TaoTag < ActiveRecord::Base
  ...
  named_scope :tag_id_and_tao_id_and_type_and_context_id, lambda { |tag_id, tao_id, tao_type, context_id| { 
              :conditions => ["tag_id = ? AND tao_id = ? AND tao_type = ? AND context_id = ?", tag_id, tao_id, tao_type, context_id] } }  
  named_scope :tag_id_and_tao_and_type_and_context_id, lambda { |tag_id, tao, tao_type, context_id| { 
              :conditions => ["tag_id = ? AND (tao_id = ? OR tao_id is null) AND tao_type = ? AND context_id = ?", tag_id, tao.id, tao_type, context_id] } }
    ...
    def create_for(tags,language_id,attributes)
      tags.map { |tag|
        ...
        if attributes[:tao_type].eql? "StatementNode"
          tao_tag = tag_id_and_tao_and_type_and_context_id(attributes[:tag_id], attributes[:tao], attributes[:tao_type], attributes[:context_id]).first || TaoTag.new(attributes)
          tao_tag
        else
