  ...
  named_scope :tag_id_and_tao_id_and_type_and_context_id, lambda { |tag_id, tao_id, tao_type, context_id| { 
    ...
    def create_for(tags,language_id,attributes)
      tags.map { |tag|
        ...
        tao_tag = tag_id_and_tao_id_and_type_and_context_id(tag_obj.id, attributes[:tao_id], attributes[:tao_type], attributes[:context_id]).first || TaoTag.new(attributes.merge({:tag => tag_obj}))
        tao_tag}.compact
    end
